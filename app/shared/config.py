...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        ...
        # Пустое значение = «не задано», а не пустая строка. Шаблон
        # ``.env.server.example`` раздаёт ключи именно так (``KEY=``), и
        # без этого флага любое незаполненное числовое поле роняет
        # приложение на старте валидацией — то есть первая же установка
    ...
    # Telegram ID владельца — единственный, кому бот отдаёт ``/backup``
    # (архив с ``.env`` и базой). Пусто — команда отключена; бот в ответ
    # подсказывает свой ID, чтобы его можно было вписать сюда.
    owner_telegram_id: int | None = None

    # Как часто бот сам присылает владельцу архив (часы). 0 — выключить.
    # Страховка для сервера, снятого на неделю: если он исчезнет раньше,
    # чем владелец вспомнит про ``/backup``, свежая копия уже лежит в
    # чате. Работает только на self-hosted (SQLite) и только когда задан
    # ``owner_telegram_id``.
