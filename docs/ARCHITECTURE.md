...
Третья задача, `purge_trash` (чистка корзины), крутится в том же цикле.
Она долго существовала только в standalone-энтрипоинте
`python -m app.workers.scheduler`, который не запускается ни на Render, ни
на своём сервере — то есть 24-часовое удержание корзины не наступало
никогда. Подключена в `app/workers/runner.py` вместе с автобэкапом
(`app/backup.py`, отправляет владельцу архив `.env` + база раз в сутки).
