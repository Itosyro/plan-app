...
>
> **Самый свежий контекст (2026-08-16, переезд между серверами):** сервер снимается на неделю, поэтому переезд сделан рутиной: `/backup` в чате отдаёт архив (`.env` + снимок SQLite), `curl … scripts/install.sh | bash -s <архив>` на новом сервере поднимает всё как было; раз в сутки архив приходит сам. Детали — верхняя запись `docs/PROGRESS.md` и [`docs/SELF-HOSTING.md`](SELF-HOSTING.md) § «Переезд». По дороге закрыт круг находок: пустое значение в `.env` роняло старт, миграции глушили все логи (бот молчал в ретрае), проверка архива не срабатывала, когда оглавление не влезало в буфер пайпа, хвосты WAL уносили восстановленные данные, русский поиск не работал на SQLite (`lower()` только ASCII), `purge_trash` не вызывался ниоткуда, после простоя напоминания шли залпом, удалённые задачи оставляли призрачные напоминания. 700 тестов. **Живьём на сервере ничего из этого не проверялось** — в песочнице нет docker.
>
> **Предыдущий контекст (2026-07-27, self-hosting):** проект можно поднять на своём VPS одной командой — `docker compose up -d`, без Render/Supabase/домена (`BOT_MODE=polling`, SQLite-файл, `AUTO_MIGRATE`). Инструкция: [`docs/SELF-HOSTING.md`](SELF-HOSTING.md), хроника — верхние две записи `docs/PROGRESS.md`. Образ собирается в GitHub Actions → GHCR (~80 МБ), на сервере только `pull`. По дороге вскрылись три немые поломки: Dockerfile вообще не собирался, первый собранный образ приехал без зависимостей, порт был привязан к localhost при внешнем прокси — все закрыты, в сборку добавлен smoke-check `import app.main`.
>
> **Незакрытое по деплою.** Сервер Юсуфа (exe.dev) публикует наружу **ровно один** порт, и он занят соседним проектом (`CopyTradeBotV2`, node на `:3002`) — отбирать нельзя, сосед может ходить через вебхук. Для этого есть профиль `router` (Caddy: `/app` и `/api` мини-аппе, остальное соседу), но он **не проверен вживую** — в песочнице нет docker. Юсуф ещё не запускал ни бота, ни проверку, свободны ли у соседа пути `/api` и `/app`. Продолжать отсюда: `docs/SELF-HOSTING.md` § «Вариант 3», шаги по порядку. На сервере попутно замечены Ollama (есть GPU) и Postgres — кандидаты на будущие задачи.
>
> **Предыдущий контекст (2026-07-27, аудит-круг 2):** прочти верхнюю запись `docs/PROGRESS.md` и `docs/audit/2026-07-27-audit-round2.md`. Круг 2 смотрел систему глазами обычного пользователя и искал регрессии круга 1: 30 находок, все подтверждены. Главное: вчерашний фикс `/today` был написан, но НЕ подключён (мёртвый хелпер) — отсюда правило в скилле `audit-loop`: **фикс без падающего без него теста считается несделанным**; закрыт CVE-2026-48710 в starlette (1.0.0 → 1.3.1); бот перестал обещать несуществующее (`/search`, «разберу позже»); документация (PLAN/ARCHITECTURE/SECURITY/.env.example/README) приведена к реальности — раньше врала почти везде. 664 теста. **Таймер goal-loop удалён по просьбе Юсуфа** — следующий круг только по его сообщению. Известный остаток: перенос задачи из списка `/today` всё ещё затирает список (см. «Осознанно отложено» в аудит-файле).
>
> **Предыдущий контекст (2026-07-26, аудит-круг 1):** прочти верхнюю запись `docs/PROGRESS.md` и `docs/audit/2026-07-26-audit.md`. Дельта в одну строку: большой многоагентный аудит → 57 подтверждённых багов, все криты исправлены (таймзоны голосового флоу, potеря подзадач в purge_trash, initData TTL, гонка кэша Mini App, пикер даты, excalidraw-чанк), модели мигрированы на `openai/gpt-oss-20b/120b` + instructor Mode.TOOLS (llama отключают в августе 2026 — откат невозможен после этого), 630 тестов. Работает **goal-loop**: каждые ~5 часов новый круг аудита свежим взглядом (скилл `.claude/skills/audit-loop`, следующий аудит-файл — `docs/audit/<дата>-audit-round2.md`). Юсуф разрешил полный автоном: пуш → PR → зелёный CI → squash-merge → deploy-check, без подтверждений.
>
