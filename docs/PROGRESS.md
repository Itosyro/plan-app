...
## 2026-07-26 — Большой аудит-круг 1: 57 багов, миграция моделей, полировка

**Контекст.** Проект долго стоял. Юсуф попросил большой аудит-ревью со
свежим взглядом: найти и исправить критические баги, улучшить
дизайн/анимации/скорость/голос, всё сверить с актуальным интернетом,
завести папку аудита и goal-loop (повторный аудит каждые ~5 часов).

**Сделано.**
- **Аудит**: многоагентный workflow (7 областей + адверсариальная
  верификация каждой находки + веб-проверка моделей Groq; 15 агентов).
  Итог: 58 находок, 57 подтверждено (7 критических). Полный отчёт —
  `docs/audit/2026-07-26-audit.md` (новая папка docs/audit/).
- **Фиксы**: все 7 критических + 27/29 важных + большинство минорных.
  Самое главное: таймзоны во всём голосовом флоу (edit_executor +
  time_resolver парсят в зоне пользователя; «послезавтра»; «в пятницу
  в 15:00»); potеря данных purge_trash (каскад детей); дайджест-гейты;
  напоминания по done-задачам и при переносе дедлайна; initData TTL
  10мин→12ч; гонка кэша Mini App; пикер даты в поясе профиля;
  excalidraw-чанк снят с холодного старта (~1.75MB→~150KB gzip).
- **Модели Groq**: llama-3.1-8b/llama-3.3-70b депрекированы (отключение
  август-2026) → `openai/gpt-oss-20b`/`gpt-oss-120b` + instructor
  Mode.TOOLS (фикс причины майского отката #171) + кэш клиентов per key.
- **Скиллы**: подключены ponytail («ленивый сеньор»), скиллы Эмиля
  Ковальского (анимации), creator-vibe; создан проектный скилл
  `.claude/skills/audit-loop`; обновлён EXTERNAL-SKILLS.md.
- **Моушен**: exit-анимации шторок, BottomNav 340→260ms, чистка
  transition-all — по стандартам emil-design-eng.
- **Доки**: docs/plans/2026-07-26-audit-improvements.md (план + путь к
  мобильному приложению: PWA → Capacitor → offline), миграция 0019.

**Верификация.** 630 pytest passed (+47), ruff format/check чисто,
mypy чисто, webapp tsc + build чисто, dist/index.html без
excalidraw-preload. Таймзонные фиксы проверены живым dateparser в
трёх зонах (+3, +7, −4).

**Не сделано (кандидаты круга 2).** Per-unit конвейеризация пайплайна;
атомарный claim дайджестов (не нужен при одном воркере); сериализация
Excalidraw на каждый onChange; restore досок из корзины; PWA/offline.
После мержа обязателен deploy-check (первый прод-выход gpt-oss).
