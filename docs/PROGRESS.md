...
## 2026-08-16 — Переезд между серверами за две команды + круг находок
...
- **Проверка архива не срабатывала на архивах больше ~64 КБ.**
  `tar | grep -q … && die`: grep закрывает пайп, tar получает SIGPIPE, и
  под `pipefail` статус становится 141 — `die` не вызывался. Подложенный
...
**Верификация.** 698 тестов (+34), ruff format/check и mypy чисто.
Отдельно: `tests/test_install_sh.py` гоняет сам скрипт под bash со
стабами git/docker/curl — переезд, битый архив, архив с `../evil`,
архив больше буфера пайпа, повторный запуск. Каждый фикс проверен
обратным экспериментом: снимаешь фикс — тест краснеет. Круговой прогон
`/backup` → распаковка на реальной схеме (19 миграций, WAL): восстановленная
база отдаёт `alembic_version = 0019` и все строки.
