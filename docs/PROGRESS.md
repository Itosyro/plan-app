...
## 2026-08-16 — Переезд между серверами одной командой + круг находок

**Контекст.** Юсуф снимает сервер на неделю и меняет его каждые ~7 дней.
Каждый раз он заново искал команды и вручную переносил ключи. Просьба:
одна команда на старом сервере, одна на новом — и всё на месте.

**Переезд (главное).**
- **`/backup` в чате** (`app/backup.py`, `app/bot/routers/commands.py`) —
  бот присылает `.tar.gz` с `.env` и снимком базы. Снимок снимается через
  online-backup API SQLite, поэтому его можно делать на живом боте.
  Команда только владельцу (`OWNER_TELEGRAM_ID`) и только в личке: в
  архиве лежат токен бота и ключи Groq, ответ в группе показал бы их всем.
- **`scripts/install.sh`** — одна команда на новом сервере:
  `curl -fsSL …/install.sh | bash -s <архив>`. Клонирует репозиторий,
  раскладывает `.env` и базу, ставит образ, поднимает бота, ждёт живого
  `/healthz`. Без архива — первая установка: создаёт `.env` из шаблона
  и просит заполнить.
- **Автобэкап раз в сутки** (`AUTO_BACKUP_HOURS`, отметка файлом
  `data/.last_backup`) — если сервер кончится раньше, чем владелец
  вспомнит про команду, свежая копия уже лежит в чате.
- **`user: "${PLAN_UID:-1000}:${PLAN_GID:-1000}"`** в compose: база живёт
  в bind-mount'е, и контейнер под чужим uid не мог её ни создать, ни
  прочитать (установка на VPS обычно из-под root). `install.sh`
  прописывает реальные id.

**Что нашли и починили по дороге** (аудит + состязательное ревью, всё
подтверждено кодом или воспроизведением):
- **Пустое значение в `.env` роняло приложение на старте.** Шаблон
  раздаёт ключи как `KEY=`, pydantic видел `""` вместо `None` и падал
  валидацией — то есть первая же установка уходила в цикл рестартов ещё
  до логов. `env_ignore_empty=True` + тест, который собирает Settings
  прямо из `.env.server.example`.
- **Миграции глушили все логи.** `fileConfig` в `alembic/env.py` шёл с
  дефолтным `disable_existing_loggers=True`, а на своём сервере схема
  поднимается прямо в старте приложения — после uvicorn и aiogram. Бот с
  неверным токеном молчал в бесконечном ретрае, а `docker compose logs`
  (единственная диагностика в инструкции) показывал пустоту.
- **Проверка архива не срабатывала, когда оглавление не влезало в буфер
  пайпа** (тысячи имён внутри, размер самого архива ни при чём).
  `tar | grep -q … && die`: grep закрывает пайп на первом совпадении, tar
  получает SIGPIPE, и под `pipefail` статус становится 141 — `die` не
  вызывался. Подложенный
  `docker-compose.override.yml` уехал бы в `docker compose up`. Оглавление
  теперь читается целиком в переменную (заодно битый архив падает ДО
  перезаписи файлов).
- **Хвосты WAL уносили восстановленные данные.** SQLite накатывает
  `plan.db-wal` на любую базу рядом: после распаковки поверх старого
  каталога данные из архива исчезали, а `integrity_check` говорил «ok».
  Перед распаковкой — `docker compose down`, снос `*-wal/-shm/-journal`,
  копия прежней базы в `plan.db.bak.<ts>`.
- **Русский поиск не работал на SQLite.** Встроенный `lower()` фолдит
  только ASCII, поэтому «Купить молоко» не находилось ни в мини-аппе, ни
  в голосовых правках («отметь …»). Юникодный `lower()` регистрируется на
  соединении (`app/db/base.py`), там же WAL и `busy_timeout`.
- **`purge_trash` не вызывался ниоткуда.** Он жил в standalone-воркере,
  который не запускается ни на Render, ни на своём сервере: обещанное
  24-часовое удержание корзины не наступало никогда. Подключён в цикл.
- **Напоминания залпом после простоя.** Первый тик после переезда слал до
  100 просроченных «⏰ Напоминаю» подряд; теперь всё старше 6 часов
  отменяется, а не отправляется.
- **Призрачные напоминания.** На SQLite нет каскадов, а `id` переиспользуется:
  напоминание от удалённой задачи цеплялось к новой и срабатывало. Зависимые
  строки удаляются явно в обоих путях удаления.
- **Бот перестал обещать то, чего нет.** Без мини-аппа (обычный случай на
  своём сервере) тексты больше не отправляют «во «Входящие» в приложении»
  и не обещают кнопку «Открыть план».
- Мелочи: заметке снова можно снять категорию (`PATCH` различает `null` и
  отсутствие ключа); `include_local_variables=False` в Sentry — иначе
  токен уезжал в трейсе; `drop_pending_updates=False` в polling — иначе
  написанное во время переезда пропадало; де-флейкнут `test_keepalive`;
  удалён мёртвый код (`ReminderInfo`, `isDefaultLayoutPrefs`).

**Верификация.** 700 тестов (+36; 702 со собранным мини-аппом), ruff format/check, mypy и tsc чисто.
Отдельно: `tests/test_install_sh.py` гоняет сам скрипт под bash со
стабами git/docker/curl — переезд, битый архив, архив с `../evil`,
архив с оглавлением больше буфера пайпа, повторный запуск поверх живой
установки, подмена `PLAN_UID`. Круговой прогон `/backup` → распаковка
закреплён тестом на реальной схеме (`test_round_trip_on_the_real_schema`:
19 миграций, WAL, снимок при открытом соединении — восстановленная база
отдаёт ту же версию схемы и те же строки). Каждый фикс проверен обратным
экспериментом: снимаешь фикс — тест краснеет; проверено для гейта
`/backup`, allow-list архива, чистки WAL, порядка «сначала погасить»,
копии базы, юникодного `lower()` и обеих правок по напоминаниям.

**Задело управляемый деплой (осознанно).** `purge_trash` теперь реально
работает и на Render — первые тики после выката вычистят накопленную
корзину старше суток (пачками по 200). Отмена протухших напоминаний тоже
общая: после простоя больше 6 часов Render перестанет досылать
просроченные — считаю это правильным поведением, а не регрессией.

**Не сделано.** Живьём на сервере не проверялось — в песочнице нет
docker-демона; профиль `router` (Caddy) по-прежнему не прогонялся.
Образ собирается только под x86-64 (задокументировано). Установку
из-под `sudo` не разводим по uid — контейнер тогда работает от root.

---

## 2026-07-27 — Доводка деплоя: три немых поломки + маршрутизатор портов

**Контекст.** Продолжение «варианта Б». Как только сборка образа стала
обязательной, вылезли дефекты, которые годами лежали незамеченными,
потому что управляемый деплой (Render) собирает проект своим
python-билдером, а Dockerfile не трогает вовсе.

**Найдено и исправлено (PR #191, #192).**
1. **Dockerfile не собирался в принципе.** `FROM node:... AS frontend
   # 20-alpine` — хвостовой комментарий Docker считает аргументами:
   «FROM requires either one or three arguments». Файл лежал в репо
   заведомо нерабочим.
2. **Первый успешно собранный образ приехал пустым** — 52 МБ, без
   единой библиотеки. `uv sync` работает с окружением *проекта*
   (`.venv`) и переменную `VIRTUAL_ENV` игнорирует, а копировался
   `/opt/venv`. Сборка «зелёная», контейнер упал бы у пользователя.
   Лечение: `UV_PROJECT_ENVIRONMENT` + **`RUN python -c "import
   app.main"` в рантайм-стадии** — теперь сборка падает в CI, если
   зависимостей нет. Второе важнее первого: закрывает класс «собралось,
   но не работает», а не конкретный случай.
3. **Порт был привязан к `127.0.0.1`** — а прокси провайдера приходит
   по сетевому интерфейсу. Снаружи выглядело бы как «сервис не
   отвечает» при бодром `ok` изнутри. Введён `APP_BIND` (PR #194).

**Маршрутизатор (PR #195).** Диагностика сервера Юсуфа показала
архитектуру, под которую пришлось подстроиться: хостинг (exe.dev)
держит свой прокси и публикует наружу по HTTPS **ровно один** порт
машины. Этот порт (3002) уже занят соседним проектом —
`node CopyTradeBotV2/src/bot.js`. Отобрать нельзя: если сосед ходит
через вебхук, он молча умрёт, а это торговый бот.

Решение — профиль `router` (Caddy, `--profile router`): занимает
публичный порт и делит трафик по путям — `/app` и `/api` мини-аппе,
всё остальное соседу по прежнему адресу. `network_mode: host`, чтобы
ходить к обоим по localhost. `APP_BIND` при этом менять не нужно:
наружу торчит только маршрутизатор — вышло безопаснее прямой
публикации. TLS не настраивается, его уже сделал прокси провайдера.

Инструкция переписана в порядке «от безопасного к необратимому»:
проверить свободны ли пути → поднять маршрутизатор на новом порту →
проверить оба сервиса → и только последним шагом переключить порт в
панели. Откат — одно движение.

**Заодно.** Починен флаки-тест `test_set_reminder_fires_at_user_local_time`:
считал «завтра» по UTC, хотя напоминание ставится на «завтра» в зоне
пользователя. Между 21:00 и 24:00 UTC в Москве уже следующие сутки —
падал бы каждый вечер; поймал CI в 21:32. Код был прав, ошибался тест.
Ирония: тест писался против сдвига суток и сам на нём споткнулся.

**Верификация.** 670 pytest, ruff/mypy/tsc чисто. Образ собран и
опубликован в GHCR: ~80 МБ при скачивании, ~250 МБ на диске (цифры в
инструкции заменены с оценки на измеренные — прежние «~1.5 ГБ»
завышали втрое и для сервера с 2 ГБ решали вопрос «ставить или нет»).

**Не проверено вживую.** Конфиг Caddy не прогонялся — в песочнице нет
docker-демона; поэтому шаг проверки в инструкции стоит ДО переключения
порта. Юсуф пока не выполнял ни запуск, ни проверку путей соседа
(`/api`, `/app` на 3002) — это первое, с чего продолжать.

**Замечено на будущее.** На сервере уже работают Ollama
(`127.0.0.1:11434`, есть GPU) и Postgres (`127.0.0.1:5432`). Кандидат
на отдельную задачу: перевод части пайплайна с облачного Groq на
локальные модели.

---

## 2026-07-27 — Self-hosting «вариант Б»: свой VPS одной командой

**Контекст.** Юсуф устал от связки внешних сервисов (Render + Supabase +
домен + вебхуки) и попросил вариант, где проект просто кладётся на его
VPS и работает. Ограничение железное: на сервере ~2 ГБ свободного диска
(остальное занято другим проектом), домена под этот проект нет.

**Сделано.**
- **Режим `BOT_MODE=polling`** (`app/main.py`): бот сам опрашивает
  Telegram исходящими запросами — не нужны ни домен, ни TLS, ни
  открытые порты. Перед стартом снимается webhook (иначе Telegram
  отвечает на `getUpdates` ошибкой 409 и бот молча немой). Дефолт
  остался `webhook`, чтобы Render-деплой не сменил поведение молча.
- **SQLite вместо Postgres**: `aiosqlite` переехал из dev- в
  runtime-зависимости; база — файл в `./data`, никакого сервиса БД.
- **`AUTO_MIGRATE=true`** + новый `app/db/migrate.py`: схема доводится
  до `head` на старте через тот же alembic-путь, что и CLI.
- **Миграция 0004 чинена под SQLite** — `alter_column` без batch-режима
  давал `near "ALTER": syntax error`, то есть на SQLite схема вообще не
  поднималась. Обёрнут в `batch_alter_table` (как уже сделано в 0010);
  на Postgres результат идентичен. Все 19 миграций проходят на SQLite.
- **Dockerfile переделан в три стадии**: компиляторы (`build-essential`,
  ~350 МБ) остались в builder-стадии и больше не едут в продакшен-образ
  — на диске в 2 ГБ это половина бюджета. Зависимости ставятся из
  `uv.lock` без dev-группы.
- **`docker-compose.yml`**: один сервис, том с базой, ротация логов
  (3×10 МБ — иначе логи однажды забьют диск), healthcheck, порт только
  на `127.0.0.1`.
- **Сборка образа в GitHub Actions** (`.github/workflows/docker.yml`) →
  GHCR. На сервере `docker compose pull` вместо сборки: локальная
  сборка тянет `node_modules` и компиляторы, которым 2 ГБ мало.
- **`.env.server.example`** — два обязательных поля (токен бота и ключи
  Groq), остальное задано в compose.
- **`docs/SELF-HOSTING.md`** — установка, обновление, бэкап, что делать
  при нехватке места, как включить мини-апп, таблица отличий от Render
  и предупреждение: два экземпляра с одним токеном одновременно
  запускать нельзя.

**Верификация.** 5 новых тестов (`tests/test_self_hosting.py`): polling
стартует и не трогает webhook; webhook снимается перед polling;
polling гасится на shutdown; webhook остаётся дефолтом;
`AUTO_MIGRATE` реально создаёт схему на пустом SQLite-файле (проверка
таблиц + `alembic_version`). Именно этот тест и поймал сломанную
миграцию 0004. Полный прогон + ruff/mypy/tsc/build — зелёные.

**Не сделано.** Мини-апп на своём сервере требует HTTPS-адреса (домен
или туннель) — это внешнее ограничение Telegram, обойти нельзя;
задокументировано. Caddy-конфиг в комплект не кладу, пока не известно,
есть ли у Юсуфа домен.

---

## 2026-07-27 — Аудит-круг 2: глазами пользователя + регрессии круга 1 + CVE

**Контекст.** Второй круг цикла «свежим взглядом». Аудиторы смотрели
систему **глазами обычного человека** (не разработчика); отдельный агент
искал регрессии, внесённые кругом 1; отдельный — проверял правдивость
документации; отдельный — свежесть стека в интернете.

**Сделано.**
- **Аудит:** 9 агентов (4 области + 4 верификатора + веб), **30 находок,
  все 30 подтверждены** (2 критических, 17 важных, 11 минорных).
  Отчёт — `docs/audit/2026-07-27-audit-round2.md`.
- **Главный урок круга:** вчерашний фикс `/today` оказался НЕ подключён —
  хелпер `_rebuild_horizon_list` написан, но не вызывался ниоткуда, при
  этом PROGRESS и аудит круга 1 уже рапортовали «исправлено». Теперь
  подключён к «выполнено» и «удалить» + тест, который падает без фикса.
  В скилл `audit-loop` внесено железное правило: фикс без падающего без
  него теста считается несделанным.
- **Регрессии круга 1 (все исправлены):** «Удалить навсегда» в Mini App
  каскадом уносило живую восстановленную подзадачу (409 вместо удаления);
  дайджест бесконечно ретраил постоянные ошибки (юзер заблокировал бота) —
  теперь различаем постоянные и временные; `mutateCache` при отсутствующей
  записи кэшировал `[]` и стирал видимый список; presence-паттерн шторки
  убил автофокус; `parse_user_datetime` не знал пользовательских якорей
  «утром/вечером».
- **Безопасность:** CVE-2026-48710 «BadHost» — `starlette` 1.0.0 → 1.3.1
  (подделанный `Host` позволял обойти path-based auth middleware).
- **Тексты бота:** убран совет несуществующей `/search`; «сохранил во
  входящие, разберу позже» заменён честным текстом + запись реально
  помечается `needs_review`; `/help` узнал про `/reminders` и объясняет,
  что такое приложение и «Входящие»; вычищены жаргон («горизонт»),
  англицизмы («medium», служебные слаги), сырые бэктики, «вы» вместо «ты»,
  «{count} задач(и)» → `plural_ru`.
- **Mini App:** пустое состояние из-за фильтров + кнопка сброса и
  индикатор в шапке; новая заметка больше не теряет текст (автозаголовок);
  выполненную задачу можно открыть; ряд «Категория» ожил + «Без
  категории»; обратимость удаления проговорена; русские названия
  горизонтов в поиске; единая терминология «Категория»; кнопка «Сегодня»
  в календаре.
- **Документы:** PLAN больше не выдаёт снятые модели за действующие и не
  утверждает «Mini App не начат»; ARCHITECTURE — реальный in-process
  scheduler вместо фантомного cron-воркера, схема БД пересобрана по
  моделям (была несуществующая `processing_jobs`, выдуманные поля
  `reminders`, отсутствовали `boards`/`task_edit_snapshots`), онбординг
  описан по факту; SECURITY — TTL 12 часов и Supabase; `.env.example`
  больше не предлагает `asyncpg`, с которым приложение не стартует;
  README актуализирован (630→664 теста, 19 миграций, Supabase, gpt-oss).

**Верификация.** 664 pytest passed (+34), ruff format/check чисто, mypy
чисто, webapp tsc + build чисто. Новые тесты включают защиту от повторения
класса дефекта: «бот не рекламирует несуществующих команд» (сканирует все
роутеры), «ни один роутер не обещает разобрать позже», AST-проверка, что
except-ветка реально ставит флаг ревью.

**Не сделано (круг 3).** Перенос задачи из списка `/today` всё ещё
затирает список (корень — шаг пикера горизонта уничтожает клавиатуру
списка; обходные пути отвергнуты как худшие); флаки-тест
`test_keepalive.py::test_loop_swallows_errors`; снятие категории у заметок
(нужен бэкенд); апгрейды aiogram 3.30 / instructor 1.15.4 (без CVE).

**Изменение процесса.** По просьбе Юсуфа автоматический таймер goal-loop
удалён — следующий круг запускается его сообщением, а не расписанием.
