...
>   названия (#150), правка категории/приоритета (#152), выключатель
>   ревью `review_enabled` в /settings (#153, миграция 0016). Эндпоинты
>   `GET /api/inbox/pending`, `POST /api/inbox/{id}/confirm`,
>   миграция 0015. Из «Voice Inbox card UX» осталось только «Разбить».
...
> - **Voice Inbox card UX** — ✅ почти готово: вкладка «Входящие» с
>   `[Подтвердить]` (#146), `[Исправить]` названия (#150), правкой
>   категории/приоритета (#152), выключателем в /settings (#153).
>   Осталось только `[Разбить]` (разбить задачу на подзадачи из карточки).
...
3. **Voice Inbox card UX** (вкладка «Входящие», вариант Б) —
   ...
   - ⬜ Осталось: `[Разбить]` задачу на подзадачи из карточки;
