...
> GitHub Actions и скачивается готовым, чтобы не собирать на сервере.
> Инструкция: [`docs/SELF-HOSTING.md`](SELF-HOSTING.md). Управляемый
> деплой на Render не меняется (дефолт остаётся webhook).
>
