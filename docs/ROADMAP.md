...
>   ревью `review_enabled` в /settings (#153, миграция 0016) и ИИ-разбивка
>   задачи на подзадачи «Разбить» (#155, эндпоинт `POST /api/tasks/{id}/split`).
>   Эндпоинты `GET /api/inbox/pending`, `POST /api/inbox/{id}/confirm`,
>   миграция 0015. **«Voice Inbox card UX» полностью закрыт.**
