...
> **Status (на 2026-05-27, после PR #106..#150):**
...
> удалён (#149).
>
> **Свежая волна (#145–#150, 2026-05-27) — «Входящие» + голосовое
> редактирование:**
> - **«Входящие» (вариант Б, #145/#146/#150)** — вместо in-chat
...
>   Там: чекбоксы keep/drop + «Подтвердить» (#146), инлайн-правка
>   названия задачи «Исправить» (#150). Эндпоинты
>   `GET /api/inbox/pending`, `POST /api/inbox/{id}/confirm`,
>   миграция 0015. Закрывает пункт «Voice Inbox card UX» из Next Up
>   (кроме «Разбить» и правки категории/приоритета во вкладке).
...
>
> **521 тест**, ruff/mypy clean, https://plan-app-t6nx.onrender.com .
>
> Все critical (C-1..C-6) и important (I-1..I-8) findings из
> `docs/REVIEW-2026-05-09-v2.md` — закрыты.
>
> **Прод-операция:**
> - Alembic migrations 0001..0015 накатаны (последние — 0013
>   `task_parent_id`, 0014 `task_completed_at`, 0015
>   `inbox_needs_review`).
...
>   недельный режим календаря, канбан по категориям, серверная
>   фильтрация календаря по диапазону месяца.
...
> - **Voice Inbox card UX** — ✅ в основном сделано: вкладка «Входящие»
>   с `[Подтвердить]` (#146) и инлайн-`[Исправить]` названия (#150).
>   Осталось: `[Разбить]` (разбить задачу на подзадачи из карточки),
>   правка категории/приоритета во вкладке, выключатель триггера в
>   /settings.
...
3. **Voice Inbox card UX** (вкладка «Входящие», вариант Б) —
   карточка с транскриптом + извлечёнными задачами.
   ...
   - ⬜ Осталось: `[Разбить]` задачу на подзадачи из карточки; правка
     категории/приоритета во вкладке; выключатель триггера в /settings;
     (опц.) ревью заметок во вкладке.
