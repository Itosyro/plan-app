...
> **Status (на 2026-07-27, self-hosting «вариант Б»):** проект теперь
> можно поднять **на своём VPS одной командой** (`docker compose up -d`)
> — без Render, Supabase и домена: `BOT_MODE=polling` (бот сам ходит в
> Telegram, входящие соединения не нужны), база — файл SQLite,
> миграции применяются на старте (`AUTO_MIGRATE`). Образ собирается в
> GitHub Actions и скачивается готовым (~80 МБ), чтобы не собирать на
> сервере. Инструкция: [`docs/SELF-HOSTING.md`](SELF-HOSTING.md).
> Управляемый деплой на Render не меняется (дефолт остаётся webhook).
>
> Для случая «хостинг публикует один порт, а он занят соседним
> проектом» есть профиль `router` (Caddy делит трафик по путям).
> **Не проверен вживую** — ждёт первого запуска на сервере; порядок
> шагов в инструкции построен так, что необратимое переключение порта
> идёт последним.
>
> **Status (на 2026-07-27, аудит-круг 2):** 30 находок глазами
> обычного пользователя + регрессии круга 1 + CVE в starlette.
> Детали: `docs/audit/2026-07-27-audit-round2.md`. Автоматический
> goal-loop **отключён** — круги запускаются по просьбе Юсуфа.
>
> **Status (на 2026-07-26, аудит-круг 1):** круг 1: 57
