...
> **СТАТУС (2026-05-26): Phase 7e ЗАВЕРШЕНА ✅** (всё смержено в `main`).
> D ✅ (#123 сегмент-контрол + bottom-sheet), A ✅ (#124 канбан=категории +
> рабочий DnD: DragOverlay/closestCorners/touch-action), C ✅ (#126 бэк
> `completed_at` + миграция 0014 / #127 фронт «Выполненные» + linger-зачёркивание
> + роут `/completed`), E2 ✅ (#125 Настройки Mira-стиль + iOS Switch),
> E-дизайн ✅ (#128 разделители канбана + базовый фон / #129 aurora «живые обои»
> белый фон + навбар Mira: чёрный неактив / синяя пилюля актив + `webapp/DESIGN.md`),
> F1 ✅ (#131 поповер «Раскладка»: вид + тумблер «Выполненные»), F2 ✅ (#137
> группировка / сортировка / фильтр + сброс), B ✅ (#132 календарь Месяц/Неделя/
> Агенда + цвета категорий + persist). **Security (G):** G1 ✅ (#133 prompt-
> injection: `wrap_untrusted` XML-делимитеры на всех LLM-стадиях + Security-секции
> в промптах + тесты) + G6 ✅ (`SECURITY.md`), G2 ✅ (#134 pin GitHub Actions по
> SHA / #139 pin Docker base images по digest + CI-джоб dependency-audit),
> G4 ✅ (#135 security-headers middleware, Telegram-WebView-safe CSP), G3 ✅
> (#138 двухшаговое подтверждение free-text delete — Excessive-Agency).
> **Осталось только G5** — намеренно отложено: scheduler/keepalive вне web-процесса
> — это trade-off под Render Free, требует решения по хостингу.
...
- [x] Канбан: колонки = категории; карточка реально перетаскивается между
  колонками пальцем и мышью; «+ колонка» создаёт категорию. (#124)
- [x] Календарь: месяц/неделя/агенда, события с цветами категорий, перенос
  drag-n-drop, persist режима. (#132)
- [x] Выполненные: linger-зачёркивание, повторный тап возвращает, экран
  «Выполненные» в настройках, `completed_at` в БД. (#126/#127)
- [x] Bottom-sheet и сегмент-контролы: явная кнопочность, ховер, активная
  обводка/пилюля, ≥44px тач-таргеты. (#123)
- [x] Поповер «Раскладка»: переключение вида + тумблер «Выполненные» +
  (F2) группировка/сортировка/фильтр, всё persist'ится. (#131/#137)
- [x] Единый Telegram-native визуал; `webapp/DESIGN.md` есть. (#128/#129)
- [x] (Workstream G) prompt-injection закрыта XML-делимитерами +
  хардненингом (#133), supply-chain запинен (#134/#139), quick-wins
  (SECURITY.md, dependency-audit CI). Также G3 (#138) + G4 (#135).
- [ ] G5 — scheduler/keepalive вне web-процесса: **намеренно отложено**
  (trade-off под Render Free, нужно решение по хостингу).
- [x] Все потоки: ruff+mypy+pytest+tsc+build зелёные; Playwright-проверки на
