...
$bad"
    ...
    # Повторный запуск на живом сервере: писать базу под работающим
    # контейнером — верный способ получить битый файл. Гасим, если есть.
...
for _ in $(seq 30); do
    ...
    # работает» от «веб-сервер жив, а опрос Telegram умер». Ждём именно
    # его, иначе скрипт рапортует успех о мёртвом боте.
    ...
    case "$health" in
        ...
        *'"status":"ok"'*)
            ...
            # выглядят точно так же. Спрашиваем лог.
