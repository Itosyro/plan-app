export function formatSummary(result: any): string {
  ...
  // Counts
  const counts: string[] = [];
  if (taskCount > 0) counts.push(`${taskCount} ${pluralize(taskCount, 'задача', 'задачи', 'задач')}`);
  if (noteCount > 0) counts.push(`${noteCount} ${pluralize(noteCount, 'заметка', 'заметки', 'заметок')}`);
  ...
  const todayTasks = (result.extractedTasks || [])
    .filter((t: any) => t.statusSuggestion === 'today')
    .slice(0, 3);
  const highPriority = (result.extractedTasks || [])
    .filter((t: any) => t.priority === 'high' && t.statusSuggestion !== 'today')
    .slice(0, 3);
  ...
  if (highPriority.length > 0 && todayTasks.length < 3) {
    ...
    parts.push('\n⭐ *Важное:*');
    highPriority.slice(0, remaining).forEach((t: any, i: number) => {
      parts.push(`${i + 1}. ${t.title}`);
    });
  }
  ...
  if (result.clarifyingQuestions?.length > 0) {
    parts.push('\n❓ Уточнения:');
    result.clarifyingQuestions.slice(0, 2).forEach((q: any) => {
