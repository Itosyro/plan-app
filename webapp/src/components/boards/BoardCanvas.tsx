...
//   • BOTTOM-RIGHT: zoom in/out/reset + mini undo.
//
// Bugs fixed vs. v1:
...
// Tools available in our custom toolbar
type ToolType = "selection" | "freedraw" | "rectangle" | "ellipse" | "arrow" | "text" | "eraser" | "hand";
...
// Draw tools = those that benefit from colour/stroke-width controls
const DRAW_TOOLS = new Set<ToolType>(["freedraw", "rectangle", "ellipse", "arrow", "text"]);

// Miro-style colour palette
const PALETTE = [
  "#1f2937", // off-black
  "#ffffff", // white
  "#22c55e", // green
...
interface StrokeWidthDef {
  value: number;
  label: string;
  thick: number; // visual indicator thickness
}
...
export function BoardCanvas({ boardId }: Props) {
  ...
  // Active tool mirrored from Excalidraw for our toolbar UI
  const [activeTool, setActiveTool] = useState<ToolType>("selection");
  // Active colour + stroke width mirrored from Excalidraw appState
  ...
  // For two-finger gesture tool-switching
  const prevToolRef = useRef<ToolType>("selection");
  const isMultiTouchRef = useRef(false);

  // For RAF-throttled CSS-var updates (dot-pattern pan/zoom tracking)
  ...
  const handleChange = useCallback(
    (
      ...
      if (appState?.currentItemStrokeWidth !== undefined) {
        setActiveStrokeWidth(appState.currentItemStrokeWidth);
      }
  ...
  // --------------------------------------------------------------------------
  // Bug #2 fix: two-finger touch → switch to hand tool to enable pinch/pan
  // --------------------------------------------------------------------------
  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;
    ...
    const handleTouchStart = (e: TouchEvent) => {
      if (e.touches.length >= 2 && !isMultiTouchRef.current) {
        ...
        const api = excalidrawApiRef.current;
        if (!api) return;
        // Remember the current active tool so we can restore it
        const curState = api.getAppState?.();
  ...
  }, [board]); // re-attach once board is loaded (containerRef is stable)
  ...
  // Undo — history.clear is the only exposed method in 0.18 (not undoOnce),
  // so we fall back to a keyboard-event dispatch for undo.
  const handleUndo = useCallback(() => {
    haptic("select");
    // Excalidraw 0.18 exposes history.clear() but not undoOnce().
    // Dispatch a synthetic Ctrl+Z to the document so Excalidraw picks it up.
    // handleKeyboardGlobally is false, so we target the canvas container.
    try {
      const el = containerRef.current;
      if (!el) return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="board-toolbar-left pointer-events-auto absolute left-3 top-1/2 z-10 -translate-y-1/2"
          ...
          <div className="pointer-events-auto absolute bottom-[calc(env(safe-area-inset-bottom,0px)+12px)] left-1/2 z-10 -translate-x-1/2">
            <div className="flex flex-col gap-2 rounded-2xl bg-tg-bg/90 px-3 py-2.5 shadow-island backdrop-blur-md">
              ...
              <div className="flex items-center gap-1.5">
                {PALETTE.map((hex) => {
                  const isActive = activeColor === hex || activeColor.toLowerCase() === hex.toLowerCase();
                  return (
                    <button
                      key={hex}
                      type="button"
                      ...
                      style={{ backgroundColor: hex, border: hex === "#ffffff" ? "1px solid rgba(0,0,0,0.12)" : undefined }}
                    />
                  );
                })}
              </div>
        ...
        <div className="pointer-events-auto absolute bottom-[calc(env(safe-area-inset-bottom,0px)+12px)] right-3 z-10 flex flex-col gap-1.5">
          ...
          <div className="flex flex-col gap-1 rounded-2xl bg-tg-bg/90 p-[6px] shadow-island backdrop-blur-md">
            ...
            >
              {/* Undo icon — simple arc arrow drawn as SVG */}
